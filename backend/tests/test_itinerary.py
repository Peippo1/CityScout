...
def test_plan_itinerary_returns_upstream_error_in_production_when_openai_client_fails(
    client,
    auth_headers,
    monkeypatch,
) -> None:
    fake_error, fake_client = _fake_openai_failure()
    monkeypatch.setattr(itinerary_service.settings, "app_env", lambda: "production")
    monkeypatch.setattr(itinerary_service.settings, "require_app_shared_secret", lambda: "test-secret")
    monkeypatch.setattr(itinerary_service, "APIConnectionError", fake_error)
    monkeypatch.setattr(itinerary_service, "OpenAI", fake_client)
    monkeypatch.setattr(itinerary_service.settings, "require_openai_api_key", lambda: "test-key")
    ...
    assert payload["error"]["message"] == "Itinerary generation is temporarily unavailable."
    assert isinstance(payload["request_id"], str)
    assert response.headers["X-Request-Id"] == payload["request_id"]
